      ...
      <a-layout-sider width="200" style="background: #fff">
        ...
        >
          <template v-for="sm in menus" :key="sm.key">
            <a-sub-menu v-if="sm.children" :key="sm.key" :title="sm.name">
              <a-menu-item v-for="m in sm.children" :key="m.key">
                <span :id="m.key">{{ m.name }}</span>
              </a-menu-item>
            </a-sub-menu>
            <template v-else>
              <a-menu-item :key="sm.key"><span :id="sm.key">{{ sm.name }}</span></a-menu-item>
            </template>
          </template>
        </a-menu>
      </a-layout-sider>
            ...
            >
              <div class="tabs">
                <a-tag
                  ...
                  :key="tab.name"
                  closable
                  :color="tabStore.active === tab.name ? 'blue' : undefined"
                  @click="to(tab)"
                  @close="tabStore.remove(tab.name)"
                  >{{ tab.title }}
                </a-tag>
              </div>
            </better-scroll>
