<template>
  ...
  >
    <template v-if="beforeBatch">
      <div>
        <p><span>操作介绍：卡片注销后将无法使用和查询</span></p>
        <p>1. 下载批量注销模板，按要求填写卡号</p>
        <p>2. 上传批量注销文件</p>
      </div>
      <div class="buttom_botton_div">
        <a-button class="buttom_botton" type="white" @click="downloadTemplate">下载批量注销模板</a-button>
        <a-upload 
          v-model:file="file" 
          :show-upload-list="false"
          :before-upload="beforeUpload"
          :customRequest="uploadFile">
          <a-button type="primary">导入批量注销文件</a-button>
        </a-upload>
      </div>
    </template>
    <template v-if="!beforeBatch">
      <div class="content_div">
        <div>注销成功：   <span>{{ successCount }}</span> 张</div>
        <div>注销失败：   <span style="color: #F65050;">{{ errorCount }}</span> 张</div>
      </div>
      <div class="buttom_botton_div">
        <a-button type="primary" @click="downloadErrorFile">下载批量注销结果</a-button>
      </div>
    </template>
  </a-modal>
</template>
...
const downloadTemplate = async() => {
  const res = await userCardApis.closeBatchTemplate();
  downloadFromRes(res);
}
...
const uploadFile = async ({ file, onSuccess, onError }) => {
  ...
  try {
    const res = await userCardApis.closeBatch(formData);
    console.log(res);
    if (res.errorCount === '0') {
      emits("success");
      onSuccess(res);
