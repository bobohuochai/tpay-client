<template>
    <div class="page page-card-request">
        <a-table bordered :columns="columns" :data-source="rechargeLogPagingRes.records" :loading="loading"
          ...
          <a @click="handleShowDetailFee(record.rechargeFee)" href="javascript:void(0)" > {{  record.rechargeFee.total }}</a>
                ...
                <div v-if="column.dataIndex === 'operation'">
                    <a v-if="userStore.userInfo.rolCode !== 'USER' &&
                        record.status === 2 &&
                        record.userId !== userStore.userInfo.id
                        " @click="showEditModal(record)">审核</a>
                </div>
            </template>
        </a-table>
